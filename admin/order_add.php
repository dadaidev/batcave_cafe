...
// Fetch clients, bookings, and menu
...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $stmt = $conn->prepare("
        ...
        VALUES (?, ?, ?, ?, ?, ?)
    ");
    $stmt->bind_param("iiidis", $client_id, $booking_id, $menu_id, $quantity, $total_amount, $special_request);
    ...
    <form method="POST">
        ...
        <select name="booking_id" class="form-control mb-3">
            ...
            <?php while($b = $bookings->fetch_assoc()): ?>
                <option value="<?= $b['booking_id'] ?>">Booking #<?= $b['booking_id'] ?> (<?= $b['booking_date'] ?>)</option>
            <?php endwhile; ?>
        </select>
        ...
        <select name="menu_id" class="form-control mb-3" required>
            ...
            <?php while($m = $menus->fetch_assoc()): ?>
                <option value="<?= $m['menu_id'] ?>"><?= $m['menu_name'] ?> (₱<?= number_format($m['menu_price'],2) ?>)</option>
            <?php endwhile; ?>
        </select>
