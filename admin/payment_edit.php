...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $stmt = $conn->prepare("
        UPDATE payment SET 
            client_id=?, booking_id=?, order_item_id=?, payment_type=?, payment_amount=?, 
            payment_method=?, payment_reference=?, payment_status=? 
        WHERE payment_id=?
    ");
    $stmt->bind_param("iiisdsssi", $client_id, $booking_id, $order_item_id, $payment_type, $payment_amount, $payment_method, $payment_reference, $payment_status, $id);
    ...
    <form method="POST">
        ...
        <select name="client_id" class="form-control mb-3" required>
            <?php while($c = $clients->fetch_assoc()): ?>
                <option value="<?= $c['client_id'] ?>" <?= $c['client_id']==$payment['client_id']?'selected':'' ?>>
        ...
        <select name="payment_type" class="form-control mb-3" required onchange="toggleFields(this.value)">
            <option value="Booking" <?= $payment['payment_type']=='Booking'?'selected':'' ?>>Booking</option>
            <option value="Order" <?= $payment['payment_type']=='Order'?'selected':'' ?>>Order</option>
        </select>
        ...
        <div id="booking_field">
            ...
            <select name="booking_id" class="form-control mb-3">
                ...
                <?php while($b = $bookings->fetch_assoc()): ?>
                    <option value="<?= $b['booking_id'] ?>" <?= $b['booking_id']==$payment['booking_id']?'selected':'' ?>>
                        Booking #<?= $b['booking_id'] ?> (₱<?= number_format($b['booking_amount'],2) ?>)
                    </option>
                <?php endwhile; ?>
            </select>
        </div>
        ...
        <div id="order_field" style="display:none;">
            ...
            <select name="order_item_id" class="form-control mb-3">
                ...
                <?php while($o = $orders->fetch_assoc()): ?>
                    <option value="<?= $o['order_item_id'] ?>" <?= $o['order_item_id']==$payment['order_item_id']?'selected':'' ?>>
                        Order #<?= $o['order_item_id'] ?> - <?= $o['menu_name'] ?> (₱<?= number_format($o['order_amount'],2) ?>)
                    </option>
                <?php endwhile; ?>
            </select>
        </div>
        ...
        <input type="number" name="payment_amount" step="0.01" class="form-control mb-3" value="<?= $payment['payment_amount'] ?>" required>
        ...
        <input type="text" name="payment_reference" class="form-control mb-3" value="<?= $payment['payment_reference'] ?>">
...
// initialize display
toggleFields("<?= $payment['payment_type'] ?>");
</script>
